...
!!! warning "Informações"
	Não será abordada a modelagem desse problema devido aos objetivos do texto. Queremos apenas ilustrar a aplicação do algoritmo de planos de corte a um problema simples com o uso da linguagem Julia. 
...
$$\begin{equation}
\sum_{j\in S}q_j \leq \sum_{j\in S}d_{jl}z_{j} + s_{l},\qquad \forall l\in T, \forall S\subseteq\{1,\dots,l\}
\end{equation},$$
...
!!! warning "Observações"
	* O solver IBM CPLEX deve ser baixado previamente e configurado;
	* Não foram realizados testes com o GLPK.
"""
...
Destinadas a leitura de instâncias (informações fornecidas) e separação para auxiliar na inclusão dos cortes. 
"""

# ╔═╡ b47e029b-960b-4897-9ad7-e22ff32a4023
function readPDLU(path::String)
	# Abrindo o arquivo da instância
	f = open(path)
...
Nessa seção apresentarei a construção do modelo e sua resolução com o uso de um solver (no caso, o CPLEX ou GLPK).
...
* _valid_: Booleano para a insersão de cortes (_true_ = sim, _false_ = não);
...
* _NumberOfUserCuts_ : Número de cortes inseridos (ou _UserCuts_).
"""
...
struct PDLU_Info
	...
	separation
	separationtime
	NumberOfUserCuts # Obs.: Não consegui obter de maneira automática (olhar documentação do CPLEX, JuMP ou MathOptInterface)
end
...
function solvePDLU(path::String; solver = CPLEX.Optimizer, valid::Bool = true)
	...
	# Desliga as saídas automáticas do solver
	# set_silent(m)
	
	# Parâmetros carregados para o CPLEX: 
	# - Desliga os cortes automáticos do CPLEX (muito eficientes)
...
# ╔═╡ f258824d-f47d-4b5f-a9da-6fe56389f7d3
solution_data.objective_value
